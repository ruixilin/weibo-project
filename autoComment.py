...
class AutoComment:
	def __init__(self, user):
		...
		logging.basicConfig(level=logging.INFO,
			...
			filename='log/publish_homepage_%s_'%(self.login_user)+self.today+'.log',
			filemode='a')
	...
	def comment(self, filename, f_comment, st1, start_page=0):
		...
		for num in range(start_page, len(mids)):
			...
			link_info = RE_link.findall(comment_contents[comment_index])[0] +"&utm_term=seeker-%s-%s"%(channel_name,str(comment_index))+"&utm_content=%s"%self.today
	...
	def commentProcess(self, user, fn2):
		...
		flag = self.comment(fn2, 'ad/ad_homepage.txt', st1, start_page)
...
if __name__ == '__main__':
	...
	f.close()
	# read in mid file
	fn2 = 'mids/mids_1084887800_20160722_20160729fa.txt'
	...
	with open('login_users.txt') as f:
		...
		for user in f.readlines():
			...
			while not os.path.exists(fn2):
				fn2 = raw_input('请输入mid列表文件路径： '.encode('gbk', 'ignore'))
			print 'start_page', start_page
			flag = commenter.comment(fn2, 'ad_test', st1, start_page)
			if flag[0] == 'editfailure':
