...
class Login:
	...
	def loginStatus(self, response):
		...
		if soup.find("div",{"class":"me"}) == None:
			...
			print u"-----------cookie已过期，重新登录！-----------".decode(code).encode('gbk')
	...
	def loginStatus_s(self, response):
		...
		if soup.find("div",{"class":"me"}) == None:
			...
			print u"-----------验证码或用户名或密码不对，请重新登录！-----------".decode(code).encode('gbk')
			self.loginSimulation()
	...
	def loginWithCookies(self, cookie_uid):
		...
		if os.path.exists(r'cookies/loginInfo_'+cookie_uid+'.txt') == False:
			print "This cookie does not exist!"
			self.loginSimulation()
			return
	...
	def loginSimulation(self):
		...
		username = raw_input(u"请输入用户名：".decode(code).encode('gbk'))
		password = raw_input(u"请输入密码：".decode(code).encode('gbk'))
